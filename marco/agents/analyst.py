...
from marco.utils import read_json, get_rm, parse_action, LLMCache
...
class Analyst(ToolAgent):
    ...
    def _is_cacheable_analyst_response(self, response: str) -> bool:
        if not isinstance(response, str) or not response.strip():
            return False

        if self.json_mode and response.strip().startswith('['):
            try:
                import json
            ...
            for command in commands:
                if not isinstance(command, dict) or 'type' not in command:
                    return False
                action_type, _argument = parse_action(json.dumps(command), json_mode=True)
                if action_type.lower() == 'invalid':
                    return False
            return True

        action_type, _argument = parse_action(response, json_mode=self.json_mode)
        return action_type.lower() != 'invalid'

    def _prompt_analyst(self, **kwargs) -> str:
        analyst_prompt = self._build_analyst_prompt(**kwargs)
        command = self.llm_cache.call(
            self.analyst,
            analyst_prompt,
            lambda: self.analyst(analyst_prompt),
            agent_name='analyst',
            no_cache=self.no_cache,
            response_validator=self._is_cacheable_analyst_response,
        )
        return command
